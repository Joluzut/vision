...
void dataRead() {
    ...
    String bitString = "";
    String bitType = "";
    int signal = digitalRead(fromNicla);// databit
    if (signal == LOW) {
        bitType += "0";
    } else {
        bitType += "1";
   ...
   delayMicroseconds(500);// Small delay if necessary
    signal = digitalRead(fromNicla);// databit
    if (signal == LOW) {
        bitType += "0";
    } else {
        bitType += "1";
   ...
   delayMicroseconds(500); // Small delay if necessary
    ...
    delayMicroseconds(500);
    signal = digitalRead(fromNicla);// check for a stop bot
      if (signal == LOW) {
      // display.clear();
...
void loop() {
  ...
  while(1){
    ...
    if (signal == HIGH) {// check for starterbit
        dataRead();
